...
- If `k.coef` is too large for the grid, the package reduces it to the
  largest supported value and gives a warning.
...
fourier_basis <- function(T, K) {
  t <- 0:(T - 1L)
  denom <- T - 1L
  if (K == 0L) return(matrix(1, nrow = T, ncol = 1L))
  cbind(
    1,
...
T  <- nrow(Y)
maxK <- as.integer(floor((T - 2L) / 2L))
Ks <- unique(as.integer(round(seq(0, maxK, length.out = 9L))))
...
In-sample reconstruction error alone cannot establish good coverage: it can
decrease as the basis fits noise. Check that substantive conclusions are stable
across plausible choices of `k.coef`.
